...
param_grid_preprocess = {
    ...
    'num_cpus': [0.5, 1, 2, 4, 8],                                                      # 0 is the default value
    'batch_size_map_batches': ['default', None, 8, 16, 32, 64, 128, 256, 512, 1024],    # 'default' is the default value
...
def evaluate(params):
    ...
    ds = ray.data.read_images(paths=image_files, mode="RGB", include_paths=True, file_extensions=None, parallelism=parallelism_read, ray_remote_args={"num_cpus": num_cpus})
    ...
    save_metrics_locally(metrics, "metrics_embl_preprocess_local.csv")
...
for config in configurations:
    ...
    save_metrics_locally(avg_metrics, f"avg_metrics_{n_repetitions}_repetitions_embl_preprocess.csv")
